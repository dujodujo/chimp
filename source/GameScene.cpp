...
void GameScene::initGame() {
	this->concrete = QPixmap(GameWidget::getResourcePath() + "/images/concrete1.png");
	if(concrete.isNull()) {
		concrete = gameStock->concrete;
	}

	this->block1 = QPixmap(GameWidget::getResourcePath() + "/images/block1.png");
	if(block1.isNull())
		block1 = gameStock->block1;

	this->block2 = QPixmap(GameWidget::getResourcePath() + "/images/block2.png");
	if(block2.isNull())
		block2 = gameStock->block2;

	this->target1 = QPixmap(GameWidget::getResourcePath() + "/images/target1.png");
	if(target1.isNull())
		target1 = gameStock->target1;

	this->target2 = QPixmap(GameWidget::getResourcePath() + "/images/target2.png");
	if(target1.isNull())
		target1 = gameStock->target1;
...
GameItem* GameScene::createGameItem(const QSet<int>& ids) {
	int ix = this->activeItems.at(qrand() % activeItems.count());
	if(ids.contains(ix)) {
	...
	for(int i = 0; i < rows; i++) {
		...
		for(int j = 0; j < columns; j++) {
			...
			if(place.empty()) {
				...
				if(place.hasTarget()) {
					painter.setOpacity(1);
					painter.drawPixmap(colx, rowy, place.place & Target1 ? target1 : target2);
				}
...
void GameScene::drawHud() {
	...
	painter.drawText(910, 10, 100, 20, Qt::AlignLeft, "Level");
}
